...
const extra = readFileSync(new URL('../tests/fixtures/typed-entity-holdout.json', import.meta.url));
cases.push(...(JSON.parse(extra.toString()) as { cases: Entry[] }).cases);
const fixtureSha256 = createHash('sha256').update(extra).digest('hex');
...
const report = { total: cases.length, passed: cases.length - failures.length, failed: failures.length, inventory: entityInventory, fixtureSha256, categories, failures };
writeFileSync(new URL('../docs/named-entity-holdout-results.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
console.log(JSON.stringify(report, null, 2));
if (cases.length < 1500 || failures.length) process.exitCode = 1;
